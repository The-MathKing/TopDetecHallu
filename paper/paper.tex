...
\begin{abstract}
Detecting hallucinations in Large Language Models (LLMs) remains a critical challenge. While prior work has explored using the hidden states of LLMs to identify ill-posed queries or hallucinations, the topological structure of the self-attention mechanism remains underexplored. In this paper, we propose an approach that applies persistent homology to the attention graphs of LLMs. We extract 0-dimensional (connected components) and 1-dimensional (cycles) topological features from the attention matrices during a single forward pass. Empirically, we find a highly asymmetric result: 0D persistent homology provides a remarkably strong, compute-efficient signal for hallucination detection that outperforms standard Maximum Softmax Probability (MSP) baselines by up to 7.8\% ROC-AUC. However, contrary to geometric intuition, 1D cyclic structure does not add complementary information and actively degrades linear classification performance, yielding near-chance predictability for non-linear classification, and retaining only a degraded linear signal. Through detailed error analysis, we identify a critical confound: 1D cycles are largely a mechanical artifact of generation sequence length, drowning out any factual grounding signal. Our findings establish 0D attention topology as a robust hallucination indicator while exposing the limitations of 1D structural reliance.
\end{abstract}
...
In this work, we extract attention matrices during a single teacher-forced forward pass and compute the Vietoris-Rips persistent homology of the resulting weighted graph. We evaluate both 0-dimensional (0D) components and 1-dimensional (1D) cycles. Our empirical results reveal a stark contrast: 0D topology provides a powerful, robust signal for hallucination detection, significantly outperforming MSP. Conversely, 1D cycles fail to provide meaningful signal and are heavily confounded by repetitive generation artifacts. 
...
\midrule
MSP (Baseline) & LR & 0.701 $\pm$ 0.023 & 0.778 $\pm$ 0.009 \\
SelfCheckGPT & LR & 0.576 $\pm$ 0.020 & 0.651 $\pm$ 0.034 \\
\midrule
0D Only & LR & \textbf{0.779 $\pm$ 0.011} & \textbf{0.708 $\pm$ 0.016} \\
 & XGB & 0.714 $\pm$ 0.012 & 0.649 $\pm$ 0.017 \\
\midrule
1D Only & LR & 0.684 $\pm$ 0.011 & 0.629 $\pm$ 0.012 \\
...
\bottomrule
\end{tabular}
\caption{Classification performance for hallucination detection across different feature sets using 5-fold cross validation. 0D persistent homology provides the strongest signal, significantly outperforming the MSP baseline.}
\label{tab:results}
\end{table}

The results show that 0D topological features (0.779 ROC-AUC) outperform both the standard Maximum Softmax Probability (MSP) baseline (0.701 ROC-AUC) and the SelfCheckGPT consistency baseline (0.576 ROC-AUC). When tested across the 5 folds via paired t-test, the improvement over SelfCheckGPT reaches marginal significance ($p = 0.050$), while the difference against MSP does not cross the standard significance threshold due to high cross-fold variance ($p = 0.133$). However, contrary to our initial hypothesis, 1D features fail to capture robust hallucination signals. In fact, training an XGBoost classifier solely on 1D features yields near-chance performance (0.502 ROC-AUC). Furthermore, concatenating 1D features with 0D features yields no statistically measurable benefit for linear models (0.779 vs 0.772, paired t-test $p = 0.165$), indicating that 1D cycles act primarily as noise or confounding variables for hallucination detection.

Crucially, this asymmetry generalizes to other model scales. As shown in the bottom rows of Table \ref{tab:results}, evaluating the smaller \texttt{Qwen2.5-1.5B-Instruct} model on a 500-example subset strictly replicates the dynamic: 0D features provide a massive, highly predictive signal (0.874 ROC-AUC with XGBoost) while 1D features fall below random chance (0.454 ROC-AUC). This confirms the 1D degradation is a consistent topological artifact of the attention mechanism, not an idiosyncratic quirk of the 3B model parameters.
